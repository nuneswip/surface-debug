...
const colorName__default = /*#__PURE__*/_interopDefaultCompat(colorName);
...
function blendColor(input, strength = 0.75) {
  ...
  if (typeof input === "string" && input.startsWith("#")) {
    ...
    if (normalized && colorName__default[normalized]) {
      rgb = colorName__default[normalized];
    } else {
      return chalk__default.white;
    }
  }
  const [r, g, b] = mixWithWhite(rgb);
  return chalk__default.rgb(r, g, b);
}
...
function _writeRawLineSync(state, line, addNewline = true) {
  ...
  if (state._activeLoading && state._spinnerInterval) {
    ...
    process__default.stdout.write("\r" + " ".repeat(width) + "\r");
    process__default.stdout.write(out);
    ...
    process__default.stdout.write("\r" + spinnerLine.padEnd(width));
  ...
  if (state._activeProgress && state._progressData) {
    ...
    const w = width || 100;
    process__default.stdout.write("\r" + " ".repeat(w) + "\r");
    process__default.stdout.write(out);
...
function createLogFunction(state) {
  return function log(level, ...args) {
    ...
    const color = chalk__default[theme[level]] || chalk__default.white;
    const msgcolor = blendColor(theme[level]) || chalk__default.white;
    ...
    const formattedLog = indent + chalk__default.gray(timestamp) + color(icon + tag) + msgcolor(message);
...
function colorizeJSON(obj) {
  return JSON.stringify(obj, null, 2).replace(/"([^"]+)":/g, (_, key) => chalk__default.gray(`"${key}"`) + ":").replace(/: "([^"]*)"/g, (_, val) => `: ${chalk__default.green(`"${val}"`)}`).replace(/: (\d+)/g, (_, num) => `: ${chalk__default.yellow(num)}`).replace(/: (true|false|null)/g, (_, val) => `: ${chalk__default.magenta(val)}`);
}
...
function loadInkfishConfigSync(cwd = process.cwd(), defaults = {}) {
  ...
  let userConfig = {};
  if (configPath) {
    const jiti$1 = jiti.createJiti(cwd, { interopDefault: true, sync: true });
    try {
      let loaded = jiti$1(configPath);
